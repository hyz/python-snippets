...
def extra_info(c, args):
    ...
    c.execute('SELECT userid,nick FROM users WHERE ' + ' or '.join(wv))
    for l in c.fetchall():
        uid = int( l['userid'] )
        print ( userd[uid] + '\t{nick}'.format(**l) )
...
def bar_activity_transfer(c, args):
    ...
    for l in f.readlines():
        ...
        m = dict( [ (x,y) for x,y in zip(names, values) if x != None ] )
...
def ExecSql(c, fmt, maps):
    def red(p, kv):
        if kv:
            ...
            p[0].append(k)
        return p
    names,values = reduce(red, maps.items(), ([],[]))
    sql = fmt.format(','.join(names) , ','.join(values))
    print (sql)
    c.execute( sql )
...
def ListData(filename, keyfield=None):
    ...
    for l in f.readlines():
        values = [ x.strip(' \r\n') for x in l.split('\t') ]
        lm.append( dict( [ (x,y) for x,y in zip(names, values) ] ) )
    if not keyfield:
        return lm
    md = {}
    for x in lm:
...
if __name__ == '__main__':
    ...
    config = ini_conf.ConfigParser()
    config.read(os.getenv("MY_CONF", "/etc/yxim.conf"))
    dbcfg  = dict(config.items('dbtest'))
